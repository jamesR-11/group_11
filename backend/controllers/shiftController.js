...
// Assign shift(s) to user (Admin only) with strategy support
const assignShift = async (req, res) => {
  try {
    ...
    // Backward compatibility: if startTime & endTime are provided, treat as single assignment
    let plan;
    if (startTime && endTime) {
      ...
      // Do not block same time for different users; check only overlapping for the same user
    ...
    // Execute plan: create one or many shifts
    const created = [];
    if (plan.type === 'single') {
      const shift = ShiftFactory.createShift(shiftType, userId, plan.startTime, plan.endTime, req.user.id, notes);
      shift.validate();
      const newShift = await shift.create();
      created.push(newShift);
    } else if (plan.type === 'batch') {
      for (const item of plan.items) {
        const shift = ShiftFactory.createShift(shiftType, userId, item.startTime, item.endTime, req.user.id, notes);
        shift.validate();
        const newShift = await shift.create();
        created.push(newShift);
      }
...
// Admin update shift details (change user, shift type, date/notes)
const adminUpdateShift = async (req, res) => {
  try {
    ...
    if (shiftType && date) {
      ...
      // If only type provided, keep the original date portion
...
const updateShiftStatus = async (req, res) => {
  try {
    ...
    // Check if user can update this shift
    if (shift.userId.toString() !== req.user.id.toString()) {
...
// Get available shift types and time ranges
